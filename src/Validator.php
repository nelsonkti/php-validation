...
class Validator
{
    use Traits\TranslationsTrait, Traits\MessagesTrait, lang\Language;
     ...
     * @return void
     */
    public function __construct($language = '',array $messages = [])
    {
    ...
    protected function registerBaseValidators()
    {
        $baseValidator = [
            ...
            'same' => new Rules\Same,
            'string' => new Rules\StringHandler,
        ...
        foreach ($baseValidator as $key => $validator) {
            if ($message = $this->translate($key)) {
                $validator = $validator->setMessage($message);
            }
